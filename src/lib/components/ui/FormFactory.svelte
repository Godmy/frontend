<script lang="ts">
  ...
  import { Input, Select, Textarea, Checkbox, Button } from './forms';
  import type { ComponentProps } from 'svelte/elements';
    ...
    definition.fields.forEach(field => {
      ...
      switch (field.type) {
        ...
        case 'password':
          ...
          if (field.required) fieldSchema = fieldSchema.min(1, field.label + ' обязательно');
          if (field.type === 'email') fieldSchema = fieldSchema.email('Некорректный email');
          break;
        case 'number':
  ...
  // Создаем store формы
  $: form = createFormStore(schema, initialValues);
